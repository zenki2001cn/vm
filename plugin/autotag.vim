...
class AutoTag:
   ...
   def updateTagsFile(self, tagsDir, tagsFile, sources):
      ...
      if self.tags_file:
         cmd = "%s --sort=yes --c-kinds=+p --c++-kinds=+p --fields=+iaS --fields=+lS --extra=+q --languages=c,c++,c#,java,python,vim,matlab,make,sh,c, --langmap=c:+.C,c++:+.H,c++:+.inl,c++:+.ipp,python:+.pyw,c:+.hlsl,c:+.vsh,c:+.psh,c:+.fx,c:+.fxh,c:+.cg,c:+.shd,c:+.glsl -f %s -a" % (self.ctags_cmd, self.tags_file)
      else:
         cmd = "%s --sort=yes --c-kinds=+p --c++-kinds=+p --fields=+iaS --fields=+lS --extra=+q --languages=c,c++,c#,java,python,vim,matlab,make,sh,c, --langmap=c:+.C,c++:+.H,c++:+.inl,c++:+.ipp,python:+.pyw,c:+.hlsl,c:+.vsh,c:+.psh,c:+.fx,c:+.fxh,c:+.cg,c:+.shd,c:+.glsl -a " % (self.ctags_cmd,)
...
EEOOFF
   " del by zenki, do not update Tlist in any type files

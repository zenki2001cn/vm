...
"Recycle path
" modify zenki, the default recycle is '.local/share/Trash/files/'. 
" set it to null if not exists.
if !exists("g:VEConf_recyclePath")
    let g:VEConf.recyclePath = ''
else
    let g:VEConf.recyclePath = g:VEConf_recyclePath

    let dir = finddir(g:VEConf.recyclePath)
    if dir == ''
        let g:VEConf.recyclePath = ''
        " exec ":silent !mkdir -p " . g:VEConf.recyclePath
    endif
endif
...
let VEConf.treePanelHotkey.SwitchToFilePanel = 'T'  " add zenki, switch TreePanel to FilePanel
...
let VEConf.filePanelHotkey.SwitchToTreePanel = 'T'  " add zenki, switch FilePanel to TreePanel
...
function! VEConf_singleFileActions['openInNewTab'](path)
    if !isdirectory(a:path)
        " modify zenki, change 'e' to open with external application.
        " exec "tabe " . g:VEPlatform.escape(a:path)
        call g:VEPlatform.start(a:path)
    else
...
function! VEConf_normalActions['openRenamer']()
    " add zenki, hide the filePanel when Renamer popup, to void filePanel may lost color
...
function! VEPlatform.copyMultiFile(fileList,topath)
    ...
    " add zenki, show copy message
    echohl Special | echomsg "Copying files to " . a:topath . " Please wait..." | echohl None
...
function! VEPlatform.copyfile(filename,topath)
    ...
    if g:VEPlatform.haswin32()
        ...
        " modify zenki, origin code exec copy process in background that can cause lost files.
        " let cmd = "cp -r " . filename . " " . topath . "&"
        let cmd = "cp -r " . filename . " " . topath
        let rtn = self.system(cmd)
...
function! s:VETreePanel.createActions()
    ...
    " add zenki, switch TreePanel to FilePanel
    exec "nnoremap <silent> <buffer> " . g:VEConf.treePanelHotkey.SwitchToFilePanel ." :call VE_SwitchToFilePanel()<cr>"
...
function! s:VEFilePanel.itemClicked(line)
    ...
    if isdirectory(path)
        ...
        " modify zenki, <cr> to open with vim
        exec "tabe " . g:VEPlatform.escape(path)
...
function! s:VEFilePanel.createActions()
    ...
    " add zenki, switch FilePanel to TreePanel
    exec "nnoremap <silent> <buffer> " . g:VEConf.filePanelHotkey.SwitchToTreePanel ." :call VE_SwitchToTreePanel()<cr>"
...
"Toggle file panel.
" modify zenki, ToggleFilePanel and setFocus
function! VE_ToggleFilePanel()
    ...
    if filePanelShow
        call s:VEContainer[winName].filePanel.setFocus()
    else
        call s:VEContainer[winName].treePanel.setFocus()
    endif
endfunction
...
" switch FilePanel to TreePanel
function! VE_SwitchToTreePanel()
    let winName = matchstr(bufname("%"),'_[^_]*$')
    if has_key(s:VEContainer,winName)
        call s:VEContainer[winName].treePanel.show()
    endif
...
"diff 2 files
" modify zenki, use DirDiff compare two directory
function! VE_Diff()
    let winNr = bufwinnr('%')
    let winName = matchstr(bufname("%"),'_[^_]*$')
    if has_key(s:VEContainer,winName)
        ...
        if len(diffFiles) != 2
            echohl WarningMsg | echo "Please select 2 files or 2 dirs to diff!" | echohl None
            return
        endif

        if !isdirectory(diffFiles[0]) && !isdirectory(diffFiles[1])
            exec "tabe " . g:VEPlatform.escape(diffFiles[0])
            exec "vertical diffsplit " . g:VEPlatform.escape(diffFiles[1])
        elseif  isdirectory(diffFiles[0]) && isdirectory(diffFiles[1])
            exec "tabe"
            exec "DirDiff " . diffFiles[0] . " " . diffFiles[1]
        else
            echohl WarningMsg | echo "Please select 2 files or 2 dirs to diff!" | echohl None
            return
        endif
